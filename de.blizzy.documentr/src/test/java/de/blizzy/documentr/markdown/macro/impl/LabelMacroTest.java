...
public class LabelMacroTest extends AbstractDocumentrTest {
	...
	@Before
	public void setUp() {
		when(beanFactory.getBean(MessageSourceMacroDescriptor.ID, "label")) //$NON-NLS-1$
			.thenReturn(new MessageSourceMacroDescriptor("label")); //$NON-NLS-1$
	}
	
	@Test
	public void getDescriptor() {
		IMacroDescriptor descriptor = macro.getDescriptor();
		assertEquals("label", descriptor.getMacroName()); //$NON-NLS-1$
		assertEquals("{{label [TYPE] TEXT/}}", descriptor.getInsertText()); //$NON-NLS-1$
	}
