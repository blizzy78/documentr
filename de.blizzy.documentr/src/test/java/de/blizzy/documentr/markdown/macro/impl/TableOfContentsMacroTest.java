...
public class TableOfContentsMacroTest extends AbstractDocumentrTest {
	...
	@Before
	public void setUp() {
		when(beanFactory.getBean(MessageSourceMacroDescriptor.ID, "toc")) //$NON-NLS-1$
			.thenReturn(new MessageSourceMacroDescriptor("toc")); //$NON-NLS-1$
	}
	
	@Test
	public void getDescriptor() {
		IMacroDescriptor descriptor = macro.getDescriptor();
		assertEquals("toc", descriptor.getMacroName()); //$NON-NLS-1$
		assertEquals("{{toc/}}", descriptor.getInsertText()); //$NON-NLS-1$
	}
