...
public class YoutubeMacroTest extends AbstractDocumentrTest {
	...
	@Before
	public void setUp() {
		when(beanFactory.getBean(MessageSourceMacroDescriptor.ID, "youtube")) //$NON-NLS-1$
			.thenReturn(new MessageSourceMacroDescriptor("youtube")); //$NON-NLS-1$
	}
	
	@Test
	public void getDescriptor() {
		IMacroDescriptor descriptor = macro.getDescriptor();
		assertEquals("youtube", descriptor.getMacroName()); //$NON-NLS-1$
		assertEquals("{{youtube [VIDEO]/}}", descriptor.getInsertText()); //$NON-NLS-1$
	}
